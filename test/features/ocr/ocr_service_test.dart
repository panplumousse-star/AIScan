...
void main() {
  ...
  group('OcrPageSegmentationMode', () {
    test('should have expected modes', () {
      ...
      expect(OcrPageSegmentationMode.values,
          contains(OcrPageSegmentationMode.auto));
      expect(OcrPageSegmentationMode.values,
          contains(OcrPageSegmentationMode.singleColumn));
      expect(OcrPageSegmentationMode.values,
          contains(OcrPageSegmentationMode.singleBlock));
      expect(OcrPageSegmentationMode.values,
          contains(OcrPageSegmentationMode.singleLine));
      expect(OcrPageSegmentationMode.values,
          contains(OcrPageSegmentationMode.singleWord));
      expect(OcrPageSegmentationMode.values,
          contains(OcrPageSegmentationMode.singleChar));
      expect(OcrPageSegmentationMode.values,
          contains(OcrPageSegmentationMode.sparseText));
      expect(OcrPageSegmentationMode.values,
          contains(OcrPageSegmentationMode.sparseTextOsd));
    });
  ...
  group('OcrOptions', () {
    ...
    group('presets', () {
      ...
      test('singleLine preset should have correct values', () {
        ...
        expect(options.language, OcrLanguage.english);
        expect(
            options.pageSegmentationMode, OcrPageSegmentationMode.singleLine);
        expect(options.engineMode, OcrEngineMode.lstmOnly);
        expect(options.preserveInterwordSpaces, true);
      });
      ...
      test('sparse preset should have correct values', () {
        ...
        expect(options.language, OcrLanguage.english);
        expect(
            options.pageSegmentationMode, OcrPageSegmentationMode.sparseText);
        expect(options.engineMode, OcrEngineMode.lstmOnly);
        expect(options.preserveInterwordSpaces, true);
      });
    ...
    group('copyWith', () {
      test('should copy with changed language', () {
        ...
        expect(copied.pageSegmentationMode,
            OcrPageSegmentationMode.auto); // Unchanged
      });
      ...
      test('should copy with all fields changed', () {
        ...
        expect(copied.language, OcrLanguage.german);
        expect(
            copied.pageSegmentationMode, OcrPageSegmentationMode.singleBlock);
  ...
  group('Integration Tests', () {
    ...
    test('should handle non-existent file path after initialization attempt',
        () async {
    ...
    test('should create options with different presets', () {
      ...
      expect(
          lineOptions.pageSegmentationMode, OcrPageSegmentationMode.singleLine);
      expect(sparseOptions.pageSegmentationMode,
    ...
    test('should support multiple language presets', () {
      final englishDoc =
          const OcrOptions.document(language: OcrLanguage.english);
  ...
  group('Error Handling', () {
    ...
    test('extractTextFromFile should throw OcrException on invalid input',
        () async {
      expect(
        () => service.extractTextFromFile(''),
        throwsA(isA<OcrException>()),
    ...
    test('extractTextFromBytes should throw OcrException on invalid input',
        () async {
      expect(
        () => service.extractTextFromBytes(Uint8List(0)),
        throwsA(isA<OcrException>()),
    ...
    test('extractTextFromMultipleFiles should throw OcrException on empty list',
        () async {
      expect(
        () => service.extractTextFromMultipleFiles([]),
        throwsA(isA<OcrException>()),
    ...
    test('extractTextWithProgress should throw OcrException on empty list',
        () async {
      expect(
        () => service.extractTextWithProgress(
          [],
  ...
  group('OcrResult word and line counting', () {
    ...
    test('should handle whitespace only text', () {
      ...
      final lineCount =
          trimmedText.isEmpty ? 0 : trimmedText.split('\n').length;
      expect(wordCount, 0);
      expect(lineCount, 0);
    });
  });
